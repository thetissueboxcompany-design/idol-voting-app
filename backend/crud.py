...
# --- User Functions (HIGHLIGHT: Updated) ---
def get_user_by_identifier(db: Session, mobile: str = None, email: str = None):
    """Fetches a user by either mobile number or email."""
    if mobile:
...
def create_user(db: Session, user: schemas.UserCreate):
    ...
    db_user = models.User(mobile_number=user.mobile_number, email=user.email)
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
    return db_user
...
# --- OTP Functions (HIGHLIGHT: Updated) ---
def create_otp(db: Session, mobile: str = None, email: str = None):
    ...
    print(f"OTP for {identifier}: {otp_code}") # For testing
    return db_otp


def verify_otp(db: Session, otp_code: str, mobile: str = None, email: str = None):
    """Verifies the provided OTP for a given identifier."""
    ...
    if mobile:
        query = query.filter(models.OTP.mobile_number == mobile)
    if email:
        query = query.filter(models.OTP.email == email)
    
    db_otp = query.order_by(models.OTP.created_at.desc()).first()
    if not db_otp:
        return None
    db_otp.is_used = True
    db.commit()
...
def get_user_vote_history(db: Session, user_id: int):
    """Fetches a detailed history of a user's votes."""
    return db.query(
        models.Vote.vote_count,
        models.Vote.created_at,
        models.Contestant.name.label('contestant_name'),
        models.VotingLine.name.label('voting_line_name'),
        models.VotingLine.start_time,
        models.VotingLine.end_time
    ).join(
        models.Contestant, models.Vote.contestant_id == models.Contestant.id
    ).join(
        models.VotingLine, models.Vote.voting_line_id == models.VotingLine.id
    ).filter(
