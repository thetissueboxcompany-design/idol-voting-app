...
def get_voting_page_state(db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    ...
    # HIGHLIGHT: The contestants now come directly from the active voting line relationship
...
# --- Admin Dashboard Endpoint (HIGHLIGHT: New section) ---
# HIGHLIGHT: This endpoint was missing from your file
...
def get_user_history(db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    """Fetches the detailed vote history for the current logged-in user."""
    ...
    for record in history_records:
        ...
        formatted_history.append(schemas.VoteHistoryDetail(
            voting_line_name=record.voting_line_name,
            voting_line_dates=date_range,
            contestant_name=record.contestant_name,
            vote_count=record.vote_count,
            voted_at=record.created_at
        ))
        
    return {"history": formatted_history}

# --- Admin Dashboard Endpoint ---
# HIGHLIGHT: This endpoint was cut off in your file
...
def get_stats_for_dashboard(
    ...
    """Fetches the vote counts for a specific voting line."""
    voting_line = crud.get_voting_line_by_id(db, line_id=line_id)
    if not voting_line:
        raise HTTPException(status_code=404, detail="Voting line not found.")
